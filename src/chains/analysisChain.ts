...
export async function runAnalysisChain(researchData: any): Promise<any> {
  ...
  const promptPath = path.join(process.cwd(), "src/prompts/analysis.txt");
  ...
  try {
    ...
    console.error("JSON parsing error in Analysis Chain. Raw content:", content);
    throw new Error(`Failed to parse analysis data: ${(error as Error).message}`);
  }
}
